...
refresh_activity();
static int
connection_inactive_check();
static int
client_listen_server_mk_sock(struct module_instance * this_module);
static int
client_has_min_ipaddr(struct module_instance * this_module);
static void
client_init_adv_time(struct client_adv_timer * ctimer);
static client_timing
client_check_adv_time(struct client_adv_timer * ctimer);
static int
client_process_request(struct module_instance * this_module,
		       char * request);
...
client_advertise_state_func(struct module_instance * this_module)
{
     ...
     if ((mc_send_sock = multicast_send_mk_sock(this_module)) < 0) {
...
client_listen_server_mk_sock(struct module_instance * this_module)
{
     ...
     client_addr.sin_addr.s_addr = this_module->addr;
...
static int
min_ip_addr_lambda(struct module_instance * client,
		   void * cmp_ipaddr)
{
	// TODO: 
	if (client->addr < *(in_addr_t*)cmp_ipaddr) {
		*(in_addr_t*)cmp_ipaddr = client->addr;
		return (-1);
	}
	return (0);
}
...
client_has_min_ipaddr(struct module_instance * this_module)
{
	in_addr_t cmp_addr = this_module->addr;
	client_list_foreach(this_module,
			    min_ip_addr_lambda,
			    &cmp_addr);
	if (this_module->addr <= cmp_addr) {
		return (1);
	} else {
		return (0);
	}
